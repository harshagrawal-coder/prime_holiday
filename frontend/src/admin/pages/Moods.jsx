...
import { FaSmile, FaEdit, FaTrash, FaPlus, FaTimes, FaToggleOn, FaToggleOff } from "react-icons/fa";

const STORAGE_KEY = "prime-holiday-masterdata-moods";

const defaultData = [
  { _id: "m-1", name: "Adventure", slug: "adventure", isActive: true, createdAt: new Date().toISOString() },
  { _id: "m-2", name: "Relaxation", slug: "relaxation", isActive: true, createdAt: new Date().toISOString() },
  { _id: "m-3", name: "Spiritual", slug: "spiritual", isActive: true, createdAt: new Date().toISOString() },
  { _id: "m-4", name: "Heritage", slug: "heritage", isActive: true, createdAt: new Date().toISOString() },
  { _id: "m-5", name: "Beach", slug: "beach", isActive: true, createdAt: new Date().toISOString() },
  { _id: "m-6", name: "Wildlife", slug: "wildlife", isActive: true, createdAt: new Date().toISOString() },
];
...
const Moods = () => {
  ...
  const handleDelete = (id) => {
    if (window.confirm("Delete this mood?")) {
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-slate-100">
            {items.length === 0 ? (
              <tr>
                <td colSpan={4} className="px-4 py-8 text-center text-sm text-slate-500">No moods yet. Add one to get started.</td>
              </tr>
            ) : (
              items.map((item) => (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-xl bg-white p-6">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="text-lg font-bold text-slate-800">{editingId ? "Edit Mood" : "Add Mood"}</h3>
              <button onClick={closeModal} className="text-slate-400 hover:text-slate-600"><FaTimes /></button>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="mb-1 block text-xs font-medium text-slate-600">Mood Name *</label>
                <input type="text" required value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                  className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-orange-500 focus:outline-none" placeholder="Adventure" />
              </div>
